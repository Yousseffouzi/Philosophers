...
void    init_fractol(t_fractal *fractal)
{
    ...
    if (fractal->mlx_connection == NULL)
        exit(EXIT_FAILURE);
    fractal->window = mlx_new_window(fractal->mlx_connection, WIDTH, HEIGHT, "Fractol");
    ...
    fractal->addr = mlx_get_data_addr(fractal->imag, &fractal->bpp, &fractal->line_len, &fractal->endian);
    data_init(fractal);
}
