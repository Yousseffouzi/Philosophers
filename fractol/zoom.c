...
int     m_zoom(int button, int x, int y, t_fractal *fractol)
{
    ...
    mouse_real = old_min_real + ((double)x / WIDTH) * (old_max_real - old_min_real);
    mouse_imag = old_min_imag + ((double)y / HEIGHT) * (old_max_imag - old_min_imag);
    if (button == 4)
		fractol->zoom *= 1.1;
	else if (button == 5)
		fractol->zoom /= 1.1;
		...
		return (0);
    update_bounds(fractol);
    fractol->offset_x += mouse_real - (fractol->min_real + ((double)x / WIDTH) * (fractol->max_real - fractol->min_real));
    fractol->offset_y += mouse_imag - (fractol->min_imag + ((double)y / HEIGHT) * (fractol->max_imag - fractol->min_imag));
    update_f(fractol);
    return (0);
}
